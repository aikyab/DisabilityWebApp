...
@router.post("/")
async def enter_survey(survey: SurveyCreate, db: Session = Depends(get_db),
...
async def get_survey(db: Session = Depends(get_db),
                    current_user: str = Depends(core_auth.get_current_user)):
...
# the operations is infact the who owns the survey data
